 ...
 * @author Ainzle
 */
public class EnemyNsPanel extends JPanel {

    private Image enemyImage;
    private List<Word> activeWordsNS;
    private Font customFont;

    public EnemyNsPanel(String ImagePath, List<Word> activeWordsNS) {
        ...
        loadCustomFont();
        setFont(customFont != null ? customFont : new Font("Roboto", Font.BOLD, 40));
    ...
    private void loadCustomFont() {
        try {
            ...
            MainManager.logError("Error with loading the font... EnemyNsPanel Game", e);  // Log the error if any  // Log the error if any
    ...
    protected void paintComponent(Graphics g) {
        ...
        synchronized (activeWordsNS) {
            for (Word word : activeWordsNS) {
                // Draw the enemy image for each word
                g.drawImage(enemyImage, word.getX() - 50, word.getY() - 50, 500, 500, this); // Draw enemy image with some offset
                ...
                // Now draw the actual text (e.g., white color)
                g.setColor(word.getTextColor()); // Set color from Word object

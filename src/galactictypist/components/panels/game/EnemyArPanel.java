 ...
 * @author Ainzle
 */
public class EnemyArPanel extends JPanel {

    private Image enemyImage;
    private List<Word> activeWordsAR;
    private Font customFont;

    public EnemyArPanel(String ImagePath, List<Word> activeWordsAR) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        synchronized (activeWordsAR) {
            for (Word word : activeWordsAR) {
                // Draw the enemy image for each word
                g.drawImage(enemyImage, word.getX() - 30, word.getY() - 30, 500, 300, this); // Draw enemy image with some offset
